...
function SignUpstep3(props) {
  ...
  const GetUserType = async () => {
    const value = await AsyncStorage.getItem('@user_Key_OtherDetails');
  ...
  const registerUser = async (params, status) => {
    ...
    const response = await fetch(APIS.register, {
      ...
      .then(res => {
        ...
        if (res.message === 'user has been saved successfully') {
          toggleEmailModal();
        } else {
  ...
  const RegistereUser = () => {
    ...
    !user_pass
      ...
      !isRegisterChecked
      ? registerWithOutBusinessAccount()
      : !company_name
    ...
    <SafeAreaView
      ...
      <Modal
        ...
        <View style={styles1.modalView}>
          ...
          <View style={styles1.successmodalBody}>
            <Text style={styles1.pendAppr}>Accept Terms & Conditions</Text>
            <Text style={styles1.anEmail}>
              Are you agree to accept all data is show in website.When
              you&rsquo;re signed in, you can always review and update
              information by visiting the services you use. For example, Photos
              and Drive are both designed to help you manage specific types of
              content you&rsquo;ve saved with our Website
            </Text>
          </View>
          <View style={styles1.btnContainer}>
            <TouchableOpacity
              style={styles1.confirmBtn}
              onPress={async () => {
                {
                  isTerm_Accepeted === false
                    ? TermsCheckHandler()
                    : toggleSuccessModal();
                }
              }}>
              <Text style={styles1.confirmText}>Agree</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
      <Modal
        ...
        <View style={styles1.modalView}>
          <View style={styles1.successmodalBody}>
            <Text style={styles1.pendAppr}>Thank You</Text>
            <Text style={styles1.anEmail}>
              Your account has been registered. We've sent you a verification
              email. Please verify your email to get your account Approved.
              Thank you
            </Text>
          </View>
          <View style={styles1.btnContainer}>
            <TouchableOpacity
              ...
              <Text style={styles1.confirmText}>Close</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const styles1 = StyleSheet.create({
  ...
  dettext: {marginTop: '5%', marginLeft: 25},
  headertext: {
    position: 'absolute',
    right: 134,
    fontSize: 19,
  ...
  headerbutton: {flexDirection: 'row', alignSelf: 'center'},
  modalContainer: {
